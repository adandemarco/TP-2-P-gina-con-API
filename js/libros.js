...
const colecciones = [
    ...
    {
        nombre: "Precursors of Science Fiction",
        consulta: "Precursors of Science Fiction"
    },
    { nombre: "Romance", consulta: "Romance" },
            ...
            if (libros_filtrados.length === 0) {
                contenedor_libros.innerHTML =
                    letra !== ""
                        ? `
                            <p>
                                No se encontraron autores que empiecen
                                con "${letra}".
                            </p>
                        `
                        : `
    ...
    libros.forEach((element) => {
        ...
        div_libro.innerHTML = `
            <img
                ...
                onerror="
                    this.onerror=null;
                    this.src='./img/portada-no-disponible.png';
                "
            >
...
function mostrar_generos_por_letra(letra) {
    ...
    if (generos_filtrados.length === 0) {
        lista_generos.innerHTML = `
            <p>
                No hay géneros que empiecen con "${letra}".
            </p>
        `;
...
function mostrar_colecciones_por_letra(letra) {
    ...
    if (colecciones_filtradas.length === 0) {
        lista_generos.innerHTML = `
            <p>
                No hay colecciones que empiecen con "${letra}".
            </p>
        `;
    ...
    colecciones_filtradas.forEach((coleccion) => {
        const boton =
            document.createElement("button");
...
btn_ingresar.addEventListener(
    "click",
    abrir_modal
);

modal_cerrar.addEventListener(
    "click",
    cerrar_modal
);

modal_overlay.addEventListener("click", (evento) => {
    if (evento.target === modal_overlay) {
        cerrar_modal();
    }
});
...
document.addEventListener("keydown", (evento) => {
    if (
        evento.key === "Escape" &&
        !modal_overlay.hasAttribute("hidden")
    ) {
        cerrar_modal();
    }
});
...
form_login.addEventListener("submit", (evento) => {
    ...
    const btn =
        document.getElementById("btn_submit_login");
...
botones_tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
        botones_tabs.forEach((otra_tab) =>
            otra_tab.classList.remove("activo")
        );
        ...
        if (filtro_actual === "generos") {
            input_busqueda.placeholder =
                "Seleccioná una letra y un género";
        ...
        if (filtro_actual === "colecciones") {
            input_busqueda.placeholder =
                "Seleccioná una letra para ver colecciones";
